...
public class ViewLancamento extends javax.swing.JFrame {
    ...
    public ViewLancamento(int _perfil, String _solicitante, String _data, String _hora, String _status, int _id_controle) {
        initComponents();
    ...
    private void carregaEspecLab() {
        listaModelEspecLab = controllerEspecLab.getListaEspecLabController(String.valueOf('1'));
    ...
    private void salvarLancamento() {
        ...
        if (isTableEmpty() == true) {
            ...
            JOptionPane.showMessageDialog(this, "Você ainda não preencheu todos os campos!", "", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private ModelResultInsp getModelResultInsp(int row) {
        ...
        modelResultInsp.setId_controle(id_controle);
...
//        Object obj = tbResultado.getValueAt(0, 13);
//        if (obj == null || obj.toString().trim().isEmpty()) {
//        } else {
        for (int i = 0; i < tbResultado.getRowCount(); i++) {
            String status_linha = tbResultado.getValueAt(i, 13).toString();
            if (status_linha.equals("Reprovado")) {
                status_controle = "Reprovado";
                break;
            } else {
                status_controle = "Aprovado";
            }
        }
//        }
    }

    public void updatStatusControle() {
        try {
            
            Connection con = new ConexaoSql().getCon();
            String query1 = "UPDATE INSP_Controle SET  "
