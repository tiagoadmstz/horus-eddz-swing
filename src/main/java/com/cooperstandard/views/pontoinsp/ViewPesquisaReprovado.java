 ...
 * @author rsouza10
 */
public class ViewPesquisaReprovado extends javax.swing.JFrame {
    ...
    public ViewPesquisaReprovado() {
        initComponents();
        carregaIdControle();
    }

    public ViewPesquisaReprovado(JFrame frame) {
        initComponents();
        carregaIdControle();
        solicitante = frame;
    }
    ...
    private void initComponents() {
        ...
        tbProblema.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Linha", "Perfil", "Solicitante", "Data Jet", "Hora Jet"
            }
        ) {
    ...
    private void tbProblemaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbProblemaMouseClicked
        if (evt.getClickCount() == 2) {
            if (solicitante != null) {
                if (solicitante instanceof ViewEntradaLancamento) {
                    ViewEntradaLancamento vp = (ViewEntradaLancamento) solicitante;
                    int linhasselecionada = tbProblema.getSelectedRow();
                    String id = tbProblema.getValueAt(linhasselecionada, 0).toString();
                    String linha = tbProblema.getValueAt(linhasselecionada, 1).toString();
                    String perfil = tbProblema.getValueAt(linhasselecionada, 2).toString();
                    String solici = tbProblema.getValueAt(linhasselecionada, 3).toString();
                    String data = tbProblema.getValueAt(linhasselecionada, 4).toString();
                    String hora = tbProblema.getValueAt(linhasselecionada, 5).toString();
                    vp.addItemCampo(id, linha, perfil, solici, data, hora);
                    dispose();
                }
            }
        }
    ...
    private void carregaIdControle() {
        listaModelInspControle = controllerInspControle.getListaInspControleController();
        ...
        for (int i = 0; i < cont; i++) {
            modelo.addRow(new Object[]{
                ...
                listaModelInspControle.get(i).getLinha(),
                listaModelInspControle.get(i).getPerfil(),
                listaModelInspControle.get(i).getSolicitante(),
                listaModelInspControle.get(i).getData_jet(),
                listaModelInspControle.get(i).getHora_jet()
            });
        }
    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ViewPesquisaReprovado.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ViewPesquisaReprovado().setVisible(true);
            }
        });
    }
